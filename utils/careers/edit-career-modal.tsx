...
import type { Career, CreateCareerData } from "./career"
...
export function EditCareerModal({ open, onOpenChange, career, onSuccess }: EditCareerModalProps) {
  const [loading, setLoading] = useState(false)
  const [newRequirement, setNewRequirement] = useState("")
  const [newSkill, setNewSkill] = useState("")
  const [newBenefit, setNewBenefit] = useState("")

  const [formData, setFormData] = useState<CreateCareerData>({
    role: "",
    experience: "",
    requirements: [],
    description: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/career", {
        ...
        body: JSON.stringify({ ...formData, id: career._id }),
      })
      ...
      toast.error("Error updating career: " + (error as Error).message);
    } finally {
      setLoading(false)
    }
  }
